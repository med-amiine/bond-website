      ...
      <div className={`marquee-track ${reverse ? 'animate-marquee-reverse' : 'animate-marquee'}`}>
        {items.map((net, i) => (
          <div key={i} className="network-tag mx-3 group">
            <span className="text-xl text-[#27279E] group-hover:scale-110 transition-transform duration-300 select-none">{net.icon}</span>
            <span className="text-[var(--text)] font-medium whitespace-nowrap text-sm">{net.name}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function Partners() {
  ...
  return (
    <section id="partners-marquee" className="relative bg-[var(--bg)] py-20 overflow-hidden">
